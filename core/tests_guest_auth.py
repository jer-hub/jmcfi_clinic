"""Tests for staff-created guest patients (no public guest login)."""
...
from core.guest_auth import GUEST_EMAIL_DOMAIN, create_guest_user, is_guest_user
...
class GuestAuthTests(TestCase):
	def test_create_guest_user(self):
		...
		self.assertTrue(user.email.endswith(f'@{GUEST_EMAIL_DOMAIN}'))
		self.assertFalse(user.has_usable_password())
		self.assertTrue(is_guest_user(user))
		profile = user.patient_profile
		self.assertTrue(profile.patient_id.startswith('G-'))
		field_names = {f.name for f in PatientProfile._meta.fields}
		self.assertNotIn('patient_category', field_names)

	def test_is_guest_user_accepts_legacy_walkin_domain(self):
		user = User.objects.create_user(
			email='walkin-legacy@walkin.local',
			password='x',
			role='patient',
		)
	...
	def test_create_guest_user_custom_names(self):
		user = create_guest_user(first_name='Ana', last_name='Reyes', phone='+639171234567')
	...
	def test_each_registration_is_new_identity(self):
		u1 = create_guest_user()
		u2 = create_guest_user()
		self.assertNotEqual(u1.pk, u2.pk)
		self.assertNotEqual(u1.email, u2.email)

	def test_guest_profile_complete_without_department(self):
		user = create_guest_user()
	...
	def test_guest_patient_id_readonly_and_ignores_post_tamper(self):
		user = create_guest_user()
		profile = user.patient_profile
		original_id = profile.patient_id
		self.assertTrue(original_id.startswith('G-'))
	...
	def test_guest_form_skips_academic_and_clears_employee_flag(self):
		user = create_guest_user()
		profile = user.patient_profile
		form = StudentProfileForm(instance=profile, user=user)
	...
	def test_guest_edit_profile_hides_academic_section(self):
		user = create_guest_user()
		client = Client()
...
class RegisterGuestPatientApiTests(TestCase):
	def setUp(self):
		self.staff = User.objects.create_user(
			email='staff-guest@test.com',
			password='pass',
			role='staff',
		)
		_complete_staff_like_profile(self.staff, 'STAFF-G-01')
		self.client = Client()
		self.client.force_login(self.staff)
		self.url = reverse('core:register_guest_patient')
	...
	def test_creates_patient_with_module_grant(self):
		...
		data = response.json()
		self.assertTrue(str(data['patient_id']).startswith('G-'))
		self.assertEqual(data['name'], 'Maria Santos')
		created = User.objects.get(pk=data['id'])
		self.assertTrue(is_guest_user(created))
