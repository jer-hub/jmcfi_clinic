...
# ──────────────────────────────────────────────────────────────────
#  F-HSS-20-0005 — Medical Certificate
# ──────────────────────────────────────────────────────────────────

def generate_medical_certificate(cert):
    """F-HSS-20-0005 — Medical Certificate."""
    doc = Document()
    _setup_page(doc)
    _add_letterhead(doc, "Medical Certificate", "F-HSS-20-0005")

    # Patient info bar
    _add_data_grid(doc, [
        ("Patient Name", cert.patient_name),
        ("Age", cert.age),
        ("Gender", cert.get_gender_display() if cert.gender else ""),
        ("Address", cert.address),
        ("Consultation Date", cert.consultation_date),
    ], cols=3)

    # Certificate body — narrative style
    _add_section_header(doc, "Certification")

    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(8)
    p.paragraph_format.space_after = Pt(4)
    p.paragraph_format.line_spacing = Pt(16)
    _run(p, "This certifies that ", size=Pt(10))
    _run(p, _val(cert.patient_name), bold=True, size=Pt(10))
    _run(p, f", {_val(cert.age)} years old, ", size=Pt(10))
    gender_text = cert.get_gender_display().lower() if cert.gender else ""
    if gender_text:
        _run(p, f"{gender_text}, ", size=Pt(10))
    _run(p, f"residing at {_val(cert.address)}", size=Pt(10))
    consult_date = cert.consultation_date.strftime("%B %d, %Y") if cert.consultation_date else "___________"
    _run(p, f", was seen and examined on {consult_date}.", size=Pt(10))

    # Diagnosis
    if cert.diagnosis:
        _add_section_header(doc, "Diagnosis")
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(4)
        _run(p, _val(cert.diagnosis), size=Pt(10))

    # Remarks & Recommendations
    if cert.remarks_recommendations:
        _add_section_header(doc, "Remarks & Recommendations")
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(4)
        _run(p, _val(cert.remarks_recommendations), size=Pt(10))

    # Certificate date
    if cert.certificate_date:
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(16)
        _run(p, "Issued this ", size=Pt(10))
        _run(p, cert.certificate_date.strftime("%d"), bold=True, size=Pt(10))
        _run(p, " day of ", size=Pt(10))
        _run(p, cert.certificate_date.strftime("%B, %Y"), bold=True, size=Pt(10))
        _run(p, ".", size=Pt(10))

    _add_signature_block(
        doc,
        name=_val(cert.physician_name),
        title_text="Physician",
        license_label="License No.",
        license_no=_val(cert.license_no),
        ptr_no=_val(cert.ptr_no),
    )

    _add_footer(doc, "F-HSS-20-0005")
    return doc
